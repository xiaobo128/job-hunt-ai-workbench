...
export function DashboardActionItems({ initialItems }: { initialItems: DashboardWorkflowItem[] }) {
  ...
  return <Panel title="近期任务" subtitle={`已完成 ${completedCount} / ${items.length} · 待处理 ${activeCount}`}>
    ...
    <div className="divide-y divide-line">
      {items.length === 0 ? <div className="rounded-2xl bg-panel p-4 text-sm text-slate-500">目前没有临近的待办或安排。</div> : items.map((item) => (
        <div key={item.id} className={`flex items-center gap-2 py-3 first:pt-0 last:pb-0 ${item.taskStatus === "COMPLETED" ? "text-slate-400" : ""}`}>
