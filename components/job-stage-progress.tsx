...
export function JobStageProgress({
  ...
  const currentIndex = isClosed ? -1 : stageNodes.findIndex((node) => node.matches.some((stage) => stage === currentStage));
  const terminalLabel = isClosed ? (hasRejectionEvent ? "已拒绝" : "已终止") : null;
