...
function NoteColumnResizeHandle({ width, onWidthChange }: { width: number; onWidthChange: (width: number) => void }) {
  ...
  return <button type="button" aria-label="调整备注列宽度" onPointerDown={(event) => { event.preventDefault(); drag.current = { pointerId: event.pointerId, startX: event.clientX, startWidth: width }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { const current = drag.current; if (!current || current.pointerId !== event.pointerId) return; onWidthChange(Math.min(420, Math.max(160, current.startWidth + event.clientX - current.startX))); }} onPointerUp={finish} onPointerCancel={finish} className="absolute -right-1 top-0 z-30 h-full w-2 cursor-col-resize touch-none rounded-sm hover:bg-accent/20 focus-visible:bg-accent/20 focus-visible:outline-none" />;
}
