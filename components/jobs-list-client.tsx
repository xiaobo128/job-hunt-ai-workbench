...
type JobItem = {
  ...
  application: { id: string; currentStage: ApplicationStage; submissionChannel: string | null; nextAction: string | null; note: string | null; updatedAt: Date } | null;
};
...
function JobEditor({ job, index, onSave, onDeleted, className, card = false }: { job: JobItem; index?: number; onSave: (e: FormEvent<HTMLFormElement>) => Promise<void>; onDeleted: (id: string) => void; className: string; card?: boolean }) {
  ...
  return <form ref={ref} onSubmit={onSave} className={className}>{app ? <><input type="hidden" name="applicationId" value={app.id} /><input type="hidden" name="submissionChannel" value={app.submissionChannel || ""} /></> : null}<div className="tabular-nums text-xs text-slate-400">{index}</div><div className="min-w-0"><Link href={`/jobs/${job.id}`} className="block truncate font-semibold text-ink underline-offset-4 hover:underline">{job.roleTitle}</Link><div className="mt-1 truncate text-slate-500">{job.companyName}{job.needsReview ? <Badge>待确认</Badge> : null}</div></div><div className="truncate text-slate-600">{job.city || "待确认"}</div>{fields}<div className="text-xs text-slate-500">{formatDate(job.updatedAt)}</div><div className="flex flex-col items-start gap-2"><Link href="/tailor" className="text-xs font-medium text-accent">Agent 准备</Link><DeleteJobForm jobLeadId={job.id} onDeleted={onDeleted} /></div></form>;
}
