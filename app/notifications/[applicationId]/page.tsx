...
import { EventStatusBadge } from "@/components/event-status-menu";
        ...
        <div className="space-y-3 border-t border-line p-3">
          {group.events.length === 0 ? <p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-500">暂无{group.label}通知。</p> : group.events.map((event) => {
            ...
            return <details key={event.id} open className={`group rounded-3xl border border-line bg-white shadow-card ${event.status === "COMPLETED" ? "opacity-70" : event.status === "IGNORED" ? "opacity-50" : ""}`}>
              <summary className="list-none cursor-pointer p-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap gap-x-3 gap-y-1 text-sm text-slate-500"><span>{getEventTypeLabel(event.eventType)}</span><span>记录于 {formatDate(event.createdAt)}</span></div><div className="mt-1"><EventTimeSummary {...event} /></div><h3 className="mt-1 truncate text-lg font-semibold text-ink">{event.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{notificationSummary(event.title, details.content)}</p></div><div className="flex shrink-0 flex-col items-end gap-2"><EventStatusBadge status={event.status} /><span className="rounded-full border border-line px-3 py-1 text-xs text-slate-500">收起</span></div></div></summary>
              <div className="border-t border-line px-4 pb-4 pt-4">
                <div className="flex items-center justify-between gap-3"><OriginalEmailDialog title={event.title} recordedAt={formatDate(event.receivedAt || event.createdAt)} content={details.content} /><NotificationEventActions status={event.status} applications={applications} event={{ id: event.id, applicationId: event.applicationId, eventType: event.eventType, title: event.title, eventTime: dateToIso(event.eventTime), windowStartAt: dateToIso(event.windowStartAt), deadlineAt: dateToIso(event.deadlineAt), receivedAt: dateToIso(event.receivedAt), relativeValidityMinutes: event.relativeValidityMinutes, content: details.content, requirements: details.requirements, artifactName: event.artifactName, artifactUrl: event.artifactUrl }} /></div>
                <section className="mt-3 rounded-3xl border border-line bg-slate-50 p-4">
                  <h4 className="text-sm font-medium text-ink">结构化字段</h4>
